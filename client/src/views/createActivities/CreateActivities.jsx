        ...
        <div className="creaActivities">
          ...
          <form className='activitieFormContainer' onSubmit={onSubmit}>
            <div className="createGrid">
                ...
                {imagenesParaEscoger.map((act)=><img  className="creaImg" key={`${act}`} src={act} alt='imagen'  onClick={()=>handleClick(act)}/> )} 
                </div>
                ...
                <div>
                  ...
                  <select id = 'selectorPaises' onChange={handleChangeSelector}>
                    <option value="">Escoja el país</option>
                      {arregloState.map((country)=>(<option key={`${country.id}`} value={`${country.id}`}>{country.nombre}</option>))}
